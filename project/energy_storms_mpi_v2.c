...
int main(int argc, char *argv[]) {
    ...
    /* then, we find the global maximum value */
        // Create a structure for MPI_MAXLOC
    struct {
        float value;
        int rank;
        int loc;
    ...
    global_maximum.value = 0.0f;
    global_maximum.loc   = 0;
    global_maximum.rank   = -1;
    local_maximum.value  = 0.0f;
    local_maximum.loc    = 0;
    local_maximum.rank    = -1;
    int max_loc_global       = 0;
    ...
    for( i=0; i<num_storms; i++) {
        ...
        float layer_last_val_recv, layer_first_val_recv ; 
        MPI_Status status;
        ...
        /* 4.2.2. Update layer using the ancillary values.
                ...
                // BE CAREFUL with the edges of the layer, doing the average we need to send and recv the neighbors values

        if (rank!=size-1){
            MPI_Send(&layer_copy[local_size], 1, MPI_FLOAT, rank + 1, 10000 + rank, MPI_COMM_WORLD);
        }

        if (rank != 0){
            MPI_Recv(&layer_last_val_recv, 1, MPI_FLOAT, rank - 1, 10000 + rank - 1, MPI_COMM_WORLD, &status);
            layer[0] = (layer_last_val_recv + layer_copy[0] + layer_copy[1] ) / 3;
        }
        ...
        if (rank!=0){
            MPI_Send(&layer_copy[0], 1, MPI_FLOAT, rank - 1, 20000 + rank, MPI_COMM_WORLD);
        }

        if (rank!=size-1){
            MPI_Recv(&layer_first_val_recv, 1, MPI_FLOAT, rank + 1, 20000 + rank + 1, MPI_COMM_WORLD,  &status);
            layer[local_size-1] = (layer_copy[local_size-2] + layer_copy[local_size-1] + layer_first_val_recv) / 3;        
        }
        ...
        for( k=1; k<local_size-1; k++ ) {
            ...
            if ( layer[k] > layer[k-1] && layer[k] > layer[k+1] ) {
                if ( layer[k] > local_maximum.value ) {
                    local_maximum.value = layer[k];
                    local_maximum.rank = rank;
                    local_maximum.loc = k;
                }
            }
        }
        ...
        // Use MPI_MAXLOC to find the global maximum value and its location
        MPI_Allreduce(&local_maximum, &global_maximum, 1, MPI_FLOAT_INT, MPI_MAXLOC, MPI_COMM_WORLD);
        
        // Let the owning processor broadcast their maximum local position to all others
        MPI_Bcast(&local_maximum.loc, 1, MPI_INTEGER, global_maximum.rank, MPI_COMM_WORLD);

        MPI_Barrier(MPI_COMM_WORLD);

        //printf(" value: %f loc: %d rank: %d", global_maximum.value, local_maximum.loc, global_maximum.rank);
        // Extract the global maximum location
        if(rank==0){
            global_maximum.loc = local_maximum.loc + global_maximum.rank * local_size;
            printf("local_maximum.loc: %d", local_maximum.loc);
        }
        //MPI_Reduce(&local_maximum, &global_maximum, 1, MPI_INT, MPI_MAXLOC, all , MPI_COMM_WORLD);
    ...
    if ( rank == 0 ) {
        ...
        printf("Result:");
        for (i=0; i<num_storms; i++)
            printf(" %d %f",  global_maximum.loc, global_maximum.value );
        printf("\n");
    }
